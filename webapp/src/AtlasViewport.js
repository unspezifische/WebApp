...
export default function AtlasViewport({ atlas, locations = [], selectedId, onSelect, onPlace, placementEnabled = false, draftMarker = null }) {
  ...
  const pointerDown = (event) => {
    if (event.button !== 0 || event.target.closest('.atlas-marker,.atlas-viewport-controls')) return;
    gestureRef.current = {
      id: event.pointerId,
      x: event.clientX,
      y: event.clientY,
  ...
  const pointerMove = (event) => {
    ...
    if (gesture.dragged) {
      ...
      setView((current) => constrain({ ...current, x: current.x + dx, y: current.y + dy }, bounds));
    }
  };
  ...
  const pointerUp = (event) => {
    ...
    if (!gesture.dragged && placementEnabled && onPlace) {
      const bounds = viewportRef.current.getBoundingClientRect();
      onPlace(
        clamp((event.clientX - bounds.left - view.x) / (bounds.width * view.zoom), 0, 1),
        clamp((event.clientY - bounds.top - view.y) / (bounds.height * view.zoom), 0, 1)
      );
    }
  };
