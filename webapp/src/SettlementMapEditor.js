...
function RoadMesh({road,strokes,heightMap,heightSampler,selected,onSelect,onInsert,onMoveStart,onMove,onMoveEnd,preview=false,presentation=false}){
  ...
  return <mesh geometry={geometry} receiveShadow {...moveProps} onClick={event=>{if(preview||(!onSelect&&!onInsert))return;event.stopPropagation();onSelect?.(road.id);if(onInsert&&(event.shiftKey||event.nativeEvent?.shiftKey))onInsert(road.id,threeToWorld(event.point));}}>
    <meshStandardMaterial color={surfaceColor} emissive={selected?'#4a3d10':'#000000'} emissiveIntensity={selected?.35:0} transparent={opacity<1} opacity={opacity} depthWrite={opacity>=.98} roughness={presentation?.9:.98} side={THREE.DoubleSide} polygonOffset polygonOffsetFactor={-2}/>
  </mesh>;
}
...
function EditorWorld({activeTool,assets,buildings,setBuildings,selected,setSelected,buildingViewMode,roads,setRoads,selectedRoadId,setSelectedRoadId,setSelectedRoadPointIndex,roadMode,buildMode,strokes,setStrokes,heightMap,setHeightMap,bounds,terrainMode,terrainMaterial,brushRadius,brushStrength,selectedAssetKey,roadDraft,setRoadDraft,roadWidth,waterBodies,waterDraft,setWaterDraft,seaLevelPicking,onSeaLevelPick,regions,setRegions,regionMode,selectedRegionPointIndex,setSelectedRegionPointIndex,fortifications,setFortifications,selectedFortificationId,setSelectedFortificationId,setSelectedFortificationPointIndex,fortificationMode,fortificationDraft,setFortificationDraft,regionDraft,setRegionDraft,selectedRegionId,setSelectedRegionId,dusk,lamps,onWaypoint,partyPosition,destination,referenceLayers,onReferencePoint,calibrationPoints,fitRequest,onMapContext,onRegionContext,onCameraChange,onScaleChange,firstPerson,onPointerLockChange,viewCommand,labelState,pointsOfInterest,virtualKeys,firstPersonSettings,touchInputRef,onInspectSelection}){
  ...
  const sculpt=(point)=>{const world=threeToWorld(point);if(lastSculpt.current&&Math.hypot(world.x-lastSculpt.current.x,world.y-lastSculpt.current.y)<brushRadius*.18)return;lastSculpt.current=world;if(!heightMap) return;
    // Create a stroke for the current brush action
    let stroke={id:`stroke-${Date.now()}-${pendingSculptStrokes.current.length}`,gesture_id:sculptGestureId.current,x:world.x,y:world.y,radius:brushRadius,brush_type:terrainMode};
    if(terrainMode==='raise'||terrainMode==='lower'){
      stroke.strength=terrainMode==='raise'?brushStrength:-brushStrength;
    }else{
      let target=flattenTarget.current;
      if(terrainMode==='smooth'){
        const sampleRadius=brushRadius*.4,samples=[[0,0],[sampleRadius,0],[-sampleRadius,0],[0,sampleRadius],[0,-sampleRadius],[sampleRadius*.7,sampleRadius*.7],[-sampleRadius*.7,sampleRadius*.7],[sampleRadius*.7,-sampleRadius*.7],[-sampleRadius*.7,-sampleRadius*.7]];
        target=samples.reduce((sum,[dx,dy])=>sum+terrainHeightAt([...strokes,...pendingSculptStrokes.current],world.x+dx,world.y+dy,heightMap),0)/samples.length;
      }
      stroke.target_elevation_feet=Number.isFinite(target)?target:world.elevation;
      stroke.strength=Math.max(.01,Math.min(1,brushStrength/100));
    }
    pendingSculptStrokes.current.push(stroke);
    
    // Apply brush to heightmap directly
    if (heightMap) {
      bakeStrokeIntoHeightmap(heightMap, stroke);
...
export default function SettlementMapEditor({activeTool,assets,buildings,setBuildings,selected,setSelected,buildingViewMode='satellite',roads,setRoads,strokes,setStrokes,heightMap=null,setHeightMap=()=>{},waterBodies=[],setWaterBodies=()=>{},mapEnvironment={},setMapEnvironment=()=>{},setFortifications=()=>{},dusk,lamps,partyPosition,destination,onWaypoint,referenceLayers=[],onReferencePoint=()=>{},calibrationPoints=[],fitRequest=0,onMapContext=()=>{},onCameraChange=null,viewCommand=null,labelState={ids:[],showAll:false},pointsOfInterest=[],campaignName=''}){
  ...
  const[virtualKeys,setVirtualKeys]=useState(new Set()),[localViewCommand,setLocalViewCommand]=useState(null),[pointerLocked,setPointerLocked]=useState(false),[scaleIndicator,setScaleIndicator]=useState({feet:100,pixels:100}),[firstPersonSettings,setFirstPersonSettings]=useState(DEFAULT_FIRST_PERSON_SETTINGS),[inspectSelection,setInspectSelection]=useState(null);
  ...
  useEffect(()=>{window.dispatchEvent(new CustomEvent('settlement-first-person-change',{detail:firstPerson}));},[firstPerson]);
