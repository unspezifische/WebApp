...
function EditorWorld({activeTool,assets,buildings,setBuildings,selected,setSelected,buildingViewMode,roads,setRoads,selectedRoadId,setSelectedRoadId,setSelectedRoadPointIndex,roadMode,strokes,setStrokes,heightMap,bounds,terrainMode,terrainMaterial,brushRadius,brushStrength,selectedAssetKey,roadDraft,setRoadDraft,roadWidth,waterBodies,waterDraft,setWaterDraft,seaLevelPicking,onSeaLevelPick,regions,setRegions,regionMode,selectedRegionPointIndex,setSelectedRegionPointIndex,fortifications,setFortifications,selectedFortificationId,setSelectedFortificationId,setSelectedFortificationPointIndex,fortificationMode,fortificationDraft,setFortificationDraft,regionDraft,setRegionDraft,selectedRegionId,setSelectedRegionId,dusk,lamps,onWaypoint,partyPosition,destination,referenceLayers,onReferencePoint,calibrationPoints,fitRequest,onMapContext,onRegionContext,onCameraChange,onScaleChange,firstPerson,onPointerLockChange,viewCommand,labelState,pointsOfInterest,virtualKeys,firstPersonSettings,touchInputRef,onInspectSelection}){
  ...
  const finishSculpt=()=>{if(pendingSculptStrokes.current.length){const additions=pendingSculptStrokes.current;pendingSculptStrokes.current=[];setStrokes(values=>[...values,...additions].slice(-1500));}lastSculpt.current=null;flattenTarget.current=null;sculptGestureId.current=null;};
