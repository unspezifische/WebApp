...
export default function SettlementManager({ headers, socket, mainEnvironmentUrl = '/', initialTool = 'atlas' }) {
  ...
  useEffect(() => {
    ...
    const bootstrap=()=>{started=true;axios.post(`/api/settlement-simulation/${campaignId}/bootstrap`, {}, { headers }).then(async ({data}) => {
  ...
  const openBuildingEvents=()=>{if(!selectedBuilding)return;const agents=economy.workforce?.agents||[],names=(selectedBuilding.occupants||selectedBuilding.present_npcs||agents.slice(0,3).map(agent=>agent.name)).filter(Boolean),worker=names[0]||selectedBuilding.owner_name||'a worker',visitor=names[1]||'a familiar local',owner=selectedBuilding.owner_name||names[2]||'the proprietor',craft=selectedBuilding.business_type||selectedBuilding.building_type||selectedAsset?.category||'trade',rumor=selectedBuilding.rumor||`unusual activity near ${selectedRoad?.name||settlementName}`;setBuildingEvents([`${worker} receives an overdue delivery for ${selectedBuilding.name}.`,`${visitor}, whom ${worker} has a crush on, enters ${selectedBuilding.name}.`,`${worker} finds a customer willing to listen to an impromptu sermon.`,`A rival ${craft} practitioner challenges ${owner} to a public duel of skill.`,`The party overhears a credible rumor about ${rumor}.`]);};
  const startBuildingEvent=event=>{setMapEnvironment(environment=>({...environment,events:[event,...(environment.events||[])].slice(0,40)}));setBuildingEvents(null);};
  ...
  return <div className="settlement-sim">
    ...
    <div className="settlement-body">
      ...
      <main className="settlement-map" onClick={()=>setMapContext(null)}>
        ...
        {activeTool!=='atlas'&&designLoaded&&!mapLoading&&<div className="map-hint">{activeTool==='travel'?'Click terrain to set a waypoint':activeTool==='build'?'Choose an asset to place, or drag the selected building and its footprint handles':activeTool==='terrain'?'Drag to sculpt terrain · WASD or arrows to pan':activeTool==='road'?'Click a road or list entry, then choose Add Points or Edit Points':activeTool==='fortification'?'Click a wall or list entry, then choose Add Points or Edit Points':activeTool==='water'?'Click a path for a river or a shoreline for lakes and oceans':activeTool==='region'?'Select a region or begin drawing a new boundary':'Select buildings for details · Scroll to zoom · WASD or arrows to pan'}</div>}
      ...
      <aside className="settlement-panel">
        ...
        <section className="panel-section building-detail"><div className="panel-kicker">SELECTED LOCATION</div>{selectedBuilding?<><div className="selected-title"><span><HomeWorkIcon/></span><div><h3>{selectedBuilding.name}</h3><p>{selectedBuilding.business_type||selectedBuilding.building_type||selectedAsset?.category||selectedAsset?.name}</p></div></div>{selectedBuilding.description&&<p className="building-description">{selectedBuilding.description}</p>}<div className="building-card-actions"><button onClick={openBuildingEditor}><EditIcon/> Edit</button><button onClick={rerollSelectedBuilding}><CasinoIcon/> Re-roll</button><button onClick={openBuildingEvents}><BoltIcon/> Event</button></div><dl><div><dt>Owner</dt><dd>{selectedBuilding.owner_name||'Unassigned'}</dd></div><div><dt>Affiliation</dt><dd>{selectedBuilding.owner_affiliation||'None'}</dd></div><div><dt>Footprint</dt><dd>{Math.round(selectedBuilding.width_feet)} × {Math.round(selectedBuilding.depth_feet)} ft</dd></div><div><dt>Front edge</dt><dd>{selectedRoad?`${selectedRoad.name} · ${selectedRoad.width_feet} ft`:'Not road-aligned'}</dd></div></dl><div className="location-focus-actions"><button type="button" onClick={()=>centerDmOn({point:{x:selectedBuilding.x,y:selectedBuilding.y,elevation:selectedBuilding.elevation||0}})}><CenterFocusStrongIcon/> Center in view</button><button type="button" onClick={()=>centerPlayerOnLocation({point:{x:selectedBuilding.x,y:selectedBuilding.y,elevation:selectedBuilding.elevation||0}})}><OpenInNewIcon/> Player View</button></div>{activeTool==='build'&&<div className="structure-actions build-only-actions"><button onClick={()=>updateSelectedBuilding(building=>({...building,rotation:building.rotation+Math.PI/2,front_road_id:null}))}>Rotate 90°</button><button className="danger-action" onClick={()=>{setBuildings(values=>values.filter(building=>building.id!==selectedBuilding.id));setSelected(null);}}>Delete</button></div>}<div className="room-list"><strong>Rooms ({selectedBuilding.rooms?.length||0})</strong><ul>{(selectedBuilding.rooms||[]).map(room=><li key={room}>{room}</li>)}</ul></div></>:<p className="empty-selection">Select a building to view its details. Map geometry remains read-only unless Build mode is active.</p>}</section>
